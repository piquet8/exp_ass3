...
*If you need you can find a more accurate description about the nodes implemented by my myself in the documentation* [docs](https://github.com/piquet8/exp_ass3/tree/main/docs)
...
The main feature of this system is the use of a finite state machine that allows greater customization of the behavior we want to obtain from the robot, in fact it is very easy to insert a new state or change the order of those present to have a different behavior from the one currently in use. There are numerous verification checks that allow the robot quick responses such as the constant check on the ready hypothesis that allows at any time to stop the search for hints to go to the home to check if the hypothesis found is the right one, this is very important because it avoids that a hypothesis is overwritten by another one before being tested. In addition there is a very good control system on the efficiency of the robot as it is possible to see at each marker found which camera detected it and the list of markers found (it is muted but there is also the possibility to see how many rooms have been visited so far), this for example allowed me to evaluate during the testing phase of the project how the cameras were behaving and how to change and improve the behavior of the robot inside the different rooms to acquire as many hints as possible and to understand which markers were the most difficult to get

## System's limitations
The main limitations of this project are the speed of execution and movement of the robot. The former limitation is probably also due to the performance of my pc, while the latter is due to the use of move_base. The main problems of this navigation system is that sometimes before reaching a room, especially at the beginning, not knowing the maze map yet, it evaluates a wrong path that leads it to enter another room and then turn around and reach the correct room, the other unexpected behavior is waiting for the actual reaching of the set target point, in fact sometimes the robot reaches the point but before declaring to have reached it waits a few seconds too long probably due to imperceptible adjustments of the robot's position and orientation. These two problems slow the achievement of the target but do not affect its final operation. One minor problem is that few markers are difficult to detec but this I think is also due to the maze lighting making them barely visible. I consider this to be of little consequence because even when the robot could not see them (about 2/3 markers) it still managed to come up with several guesses including the winning one; in the next paragraph, however, I thought about what might be a possible solution.

## System's technical improvements

The improvements for this project are definitely focused on making the robot's movement smoother and more efficient, to do this one could use the combination of other autonomous navigation planner and maybe modify the parameters currently used for move_base. Instead, as far as detecting and collecting hints (and thus markers) is concerned, I am quite satisfied with the system I implemented, this is because on the first round of the maze the robot after visiting all the rooms acquires no less than 26 markers, and then almost always finds the missing ones in the following rounds if necessary to find the hypothesis. However in order to further improve this aspect I thought of some tricks, first of all at the code level I should implement only one node for all 3 rooms regarding the detect function. At the robot behavior level instead I thought that I could add a light source to the robot to allow it to illuminate those markers that are barely visible (a flashlight in true Sherlock Holmes style), also I could implement lists and counters for the individual rooms to keep track of the hints and markers collected in the individual visits so that I can use this infomrmation in the next round. If I know that for example in 4 rooms all the hints have been collected I can force the robot to visit only the rooms with missing markers and maybe tell it to use a different behavior in the room than it used in the previous visit (e.g. do a rotation to another point in the room). Finally, the last modification would be to turn off the 'marker detecting behavior' while moving between rooms to make the simulation more similar to what would be the behavior of the investigator; at the moment in fact the robot founds hints even while moving from one room to another
